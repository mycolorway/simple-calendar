...
Calendar = (function(superClass) {
  ...
  Calendar.prototype._processEvent = function(originEvent) {
    ...
    if (!moment.isMoment(event.start)) {
      event.start = this.moment(event.start, 'YYYY-MM-DDTHH:mm:ssZ');
    }
    if (!moment.isMoment(event.end)) {
      event.end = this.moment(event.end, 'YYYY-MM-DDTHH:mm:ssZ');
    }
  ...
  Calendar.prototype.addEvent = function(events) {
    ...
    for (j = 0, len = events.length; j < len; j++) {
      ...
      if (!(this.dateInMonth(event.start) || this.dateInMonth(event.end) || this.month.isBetween(event.start, event.end, 'month'))) {
        continue;
      }
      if (event.acrossDay) {
