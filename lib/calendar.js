(function (root, factory) {
  if (typeof define === 'function' && define.amd) {
    ...
    define('simple-calendar', ["jquery",
      "simple-module",
      "simple-dragdrop",
      "moment-timezone"], function ($, SimpleModule, SimpleDragdrop, moment) {
      return (root.returnExportsGlobal = factory($, SimpleModule, SimpleDragdrop, moment));
    });
...
Calendar = (function(_super) {
  ...
  Calendar.prototype._bindDrag = function() {
    var dragdrop;
    dragdrop = this.el.data('dragdrop');
    dragdrop.on('dragenter', function(e, event, target) {
      ...
      return $('.day').slice(index, days + index + 1).addClass('dragover');
    });
    dragdrop.on('dragstart', function(e, event) {
      var $event;
      $event = $(event);
      event = $event.data('event');
      if (!event) {
      ...
      return this.trigger('eventdragstart', event);
    });
    dragdrop.on('dragend', function(e, event) {
      var $event, $events;
      $event = $(event);
      event = $event.data('event');
      if (!event) {
      ...
      $('.day').removeClass('dragover');
      return $('.days').css('cursor', 'default');
    });
    return dragdrop.on('drop', (function(_this) {
      return function(e, event, target) {
        ...
        return _this.trigger('eventdragend', event);
      };
    })(this));
  };
