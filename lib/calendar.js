...
Calendar = (function(superClass) {
  ...
  Calendar.prototype._bind = function() {
    ...
    this.el.on('mouseenter.calendar', '.todo', (function(_this) {
      return function(e) {
        var $todo, todo;
        $todo = $(e.currentTarget);
        todo = $todo.data('todo');
        return _this.trigger('todomouseenter', [todo, $todo]);
      };
    })(this));
    return this.el.on('mouseleave.calendar', '.todo', (function(_this) {
      return function(e) {
        var $todo, todo;
        $todo = $(e.currentTarget);
        todo = $todo.data('todo');
        return _this.trigger('todomouseleave', [todo, $todo]);
      };
    })(this));
  };

  Calendar.prototype._initDrag = function() {
    var dragdrop, draggable;
    if (!(simple.dragdrop && this.opts.allowDrag !== 'disable')) {
      return;
    }
    if (this.opts.draggable) {
      draggable = this.opts.draggable;
    } else if (this.opts.allowDrag === 'all') {
    ...
    dragdrop = simple.dragdrop({
      ...
      helper: function($event) {
        ...
        $helper = $event.clone();
        event = $event.data('event') || $event.data('todo');
        if (!event.start) {
      ...
      placeholder: function($event) {
        ...
        if (event.acrossDay) {
          ...
          if ($event.data('todo')) {
            $events = this.el.find(".todo[data-id='" + event.id + "']:not(.drag-helper)");
          }
          $events.hide();
        }
        return null;
    ...
    dragdrop.on('dragend', (function(_this) {
      return function(e, event) {
        ...
        if (event.acrossDay) {
          ...
          if ($event.data('todo')) {
            $events = _this.el.find(".todo[data-id='" + event.id + "']:not(.drag-helper)");
          }
          $events.show();
        }
        setTimeout(function() {
    ...
    return dragdrop.on('drop', (function(_this) {
      return function(e, event) {
        ...
        if ($event.data('event')) {
          ...
          _this.replaceTodo(event);
          return _this.trigger('tododrop', [event, differ]);
        }
      };
    })(this));
  };
  ...
  Calendar.prototype._sortItems = function(items) {
    items.sort(function(item1, item2) {
      ...
      if (result === 0) {
        result = _differentInCalendarDays(item2.end, item1.start) - _differentInCalendarDays(item1.end, item2.start);
      }
      if (result === 0) {
  ...
  Calendar.prototype.addEvent = function(events) {
    ...
    if (eventsInDay.length > 0) {
      ...
      this.events.inDay = _sortItems(this.events.inDay);
    ...
    if (eventsAcrossDay.length > 0) {
      ...
      this.events.acrossDay = _sortItems(this.events.acrossDay);
  ...
  Calendar.prototype.replaceEvent = function(newEvent) {
    ...
    if (event.acrossDay) {
      this.el.find('.event-spacers').empty();
      this.el.find(".event[data-id=" + newEvent.id + "]").remove();
      ref = this.events.acrossDay;
      results = [];
      for (j = 0, len = ref.length; j < len; j++) {
        e = ref[j];
  ...
  Calendar.prototype._processTodo = function(originTodo) {
    ...
    if (todo.end && !moment.isMoment(todo.end)) {
  ...
  Calendar.prototype.addTodo = function(todos) {
    ...
    if (todosInDay.length > 0) {
      ...
      this.todos.inDay = _sortItems(this.todos.inDay);
    ...
    if (todosAcrossDay.length > 0) {
      ...
      this.todos.acrossDay = _sortItems(this.todos.acrossDay);
  ...
  Calendar.prototype.removeTodo = function(todo) {
    var arr, index, j, key, len, ref, t, todoId;
  ...
  Calendar.prototype.replaceTodo = function(newTodo) {
    ...
    $.extend(todo, newTodo);
    todo.acrossDay = todo.start && (this._DiffDay(todo.end, todo.start) !== 0) ? true : false;
    this.el.find(".event-spacer[data-id=" + todo.id + "]").remove();
    if (todo.acrossDay) {
      this.el.find(".todo[data-id=" + todo.id + "]").remove();
      return this._renderAcrossDay(todo, 'todo');
    } else {
